...
//  GraduationProj
//
//  Created by Naira Bassam on 03/07/2021.
//
...
//        }
    ...
    func powerSpectra(_ array:[Double], _ fps: Double) -> [Double]{
        var resultPS: [Double] = []
        let N = array.count
        //(abs(fft()))^2 / (fs*N)
        func powerFFT(_ value: Double) -> Double{
            let power = pow(abs(value), 2)
            let result = power/(fps*Double(N))
            return result
        }
        for index in 0..<N{
            resultPS.append(powerFFT(array[index]))
        }
        return resultPS
    }
}
